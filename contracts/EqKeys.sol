...
contract EQKEYS is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint public presaleCost = .22 ether;
    uint public publicSaleCost = .33 ether;
    uint public maxSupply = 264;
    uint public presaleMintAmount = 2;
    uint public publicSaleMintAmount = 4;
    uint public presaleKeysPerAddressLimit = 2;
    uint public keysPerAddressLimit = 4;
	...
	//only owner
	function penthouseMint(uint _mintAmount) public onlyOwner {
		uint supply = totalSupply();
		require(_mintAmount > 0, "at least 1 KEY needs to be minted");
